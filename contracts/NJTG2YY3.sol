    ...
    SaleState
{
    ...
    string private _baseURI_ =
        "https://jfmc-staging-api-hxs7r5kyjq-uc.a.run.app/";
    ...
    modifier maxSupplyLimit(uint256 quantity_) {
        uint256 limit = MAX_SUPPLY - reserved - _totalMinted();
    ...
    {
        // Only 3333 Mints
        if (allowListQuantity >= ALLOW_LIST_SUPPLY) revert SoldOut();

        if (getAllowListUsed(_msgSender()) + quantity_ > 2)
        ...
        )
    {
        ...
        uint8 usedAllowance = getHoldersUsed(_msgSender());
    ...
    function _setAllowListUsed(uint8 quantity) private {
        uint64 packed = _getAux(_msgSender());
        _setAux(_msgSender(), (packed & (~uint64(3))) | (uint64(quantity)));
    }

    function getAllowListUsed(address address_) public view returns (uint8) {
        uint64 packed = _getAux(address_);
        return uint8(packed & uint64(3));
    }

    function _setHoldersGuaranteeUsed() private {
        uint64 packed = _getAux(_msgSender());
        _setAux(_msgSender(), packed | (uint64(1) << 2));
    }
    ...
    {
        uint64 packed = _getAux(address_);
        return ((packed >> 2) & uint64(1)) == 1 ? true : false;
    }
    ...
    function _setHoldersUsed(uint8 quantity) private {
        ...
        _setAux(
            _msgSender(),
            (packed & (~uint64(255) << 3)) | (uint64(quantity) << 3)
        );
    }

    function getHoldersUsed(address address_) public view returns (uint8) {
        uint64 packed = _getAux(address_);
        return uint8((packed >> 3) & uint64(255));
    }
