    ...
    ReentrancyGuard
{
    ...
    string private _name = "NFXHG2Q1";
    string private _symbol = "NFXHG2Q1";
    ...
    IJungleFreaksNFT private _freaksContract;
    IERC1155 private _vaultContract;

    constructor(IJungleFreaksNFT freaksContract_, IERC1155 vaultContract_) {
        _freaksContract = freaksContract_;
    ...
    function getAllRewards(address staker) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            totalRewards +=
                ((getStakingTimestamp() - timeStaked[tokens[i]]) *
                    EMISSION_RATE) /
                86400;
        }
        ...
        returns (uint256)
    {
        ...
        totalRewards +=
            ((((timestamp - stakingState.cotfLastStaked) *
                stakingState.cotfStaked) + stakingState.cotfAccumulatedTime) *
                EMISSION_RATE_COTF) /
            86400;
        totalRewards +=
            ((((timestamp - stakingState.mtfmLastStaked) *
                stakingState.mtfmStaked) + stakingState.mtfmAccumulatedTime) *
                EMISSION_RATE_MTFM) /
            86400;
    ...
    function stakeById(uint256[] calldata tokenIds) external stakingEnabled {
        require(
            stakerTokens[msg.sender].length + tokenIds.length <= 100,
            "MAX_TOKENS_STAKED"
        );
        uint256 timestamp = getStakingTimestamp();
    ...
    function stakeLegendaries(uint8 cotf, uint8 mtfm)
        external
        stakingEnabled
        nonReentrant
    {
        ...
        if (cotf > 0) {
            stakingState.cotfAccumulatedTime +=
                (uint32(timestamp) - stakingState.cotfLastStaked) *
                stakingState.cotfStaked;
            stakingState.cotfLastStaked = uint32(timestamp);
            stakingState.cotfStaked += cotf;

            _vaultContract.safeTransferFrom(
                msg.sender,
                address(this),
                LEGENDARY_COTF,
                cotf,
        ...
        if (mtfm > 0) {
            stakingState.mtfmAccumulatedTime +=
                (uint32(timestamp) - stakingState.mtfmLastStaked) *
                stakingState.mtfmStaked;
    ...
    function unstakeLegendaries(uint8 cotf, uint8 mtfm) external nonReentrant {
        ...
        if (cotf > 0) {
            totalRewards +=
                ((((timestamp - stakingState.cotfLastStaked) *
                    stakingState.cotfStaked) +
                    stakingState.cotfAccumulatedTime) * EMISSION_RATE_COTF) /
                86400;
        ...
        if (mtfm > 0) {
            totalRewards +=
                ((((timestamp - stakingState.mtfmLastStaked) *
                    stakingState.mtfmStaked) +
                    stakingState.mtfmAccumulatedTime) * EMISSION_RATE_MTFM) /
                86400;
    ...
    function unstakeByIds(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards +=
                ((timestamp - timeStaked[id]) * EMISSION_RATE) /
                86400;
    ...
    function updateEmissionEnd(uint256 newTime) external {
        EMISSION_END = newTime;
    }

    function getStakingTimestamp() internal view returns (uint256) {
    ...
    function setController(address controller, bool authorized) public {
